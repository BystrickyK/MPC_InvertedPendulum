function [A,B,C,D] = getABCD(X, u, p)

    %B_eq,B_p,J_eq,J_p,K_g,M_p,R_m,eta_g,eta_m,g,k_m,k_t,l_p,r_mp
    ...
    B = [0.0;0.0;(1.0./r_mp.^2.*(J_p.*K_g.*eta_g.*eta_m.*k_t.*r_mp+K_g.*M_p.*eta_g.*eta_m.*k_t.*l_p.^2.*r_mp))./(R_m.*(M_p.^2.*l_p.^2+J_eq.*J_p+J_p.*M_p-M_p.^2.*l_p.^2.*cos(X(2)).^2+J_eq.*M_p.*l_p.^2));-(K_g.*M_p.*eta_g.*eta_m.*k_t.*l_p.*cos(X(2)))./(R_m.*r_mp.*(M_p.^2.*l_p.^2+J_eq.*J_p+J_p.*M_p-M_p.^2.*l_p.^2.*cos(X(2)).^2+J_eq.*M_p.*l_p.^2))]
    C = [1 0 0 0;
        0 1 0 0];
    D = 0;
end